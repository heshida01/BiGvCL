...
class DataHandler:
    ...
    def load_data_from_database(self, dataset, mode='', testing=True, relation_map=None,
        ...
        """
        print("args.data_seed",args.data_seed)
        dtypes = {
            'd_nodes': np.str_, 'g_nodes': np.int32,
            'relations': np.float32}

        # Adjust path to data directory
        data_dir = '../data' if 'src' in os.getcwd() else './data'
        filename_train = os.path.join(data_dir, dataset, mode, 'train.csv')
        filename_test = os.path.join(data_dir, dataset, mode, 'test.csv')
    ...
    def LoadData(self):
        ...
        self.trnLoader = dataloader.DataLoader(trnData, batch_size=args.batch, shuffle=True,
                                               num_workers=0)  # already shuffled training set

        valData = TstData(val_labels, d_val_idx, g_val_idx)
